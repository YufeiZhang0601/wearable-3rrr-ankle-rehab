...
\title{Design, Kinematic Optimization, and Assist-As-Needed Control of a Wearable 3-RRR Spherical Parallel Ankle Rehabilitation Robot}
...
\thanks{This report was prepared for the AMR Research Course, May 2026. The accompanying simulation, kinematic, and control source code are publicly available at \url{https://github.com/YufeiZhang0601/rh1}.}
}

\maketitle

\begin{abstract}
Conventional ankle rehabilitation devices either rely on therapist-guided manual therapy, which is difficult to standardize, or on bulky multi-degree-of-freedom platforms whose moving center of rotation seldom coincides with the anatomical ankle joint. The resulting kinematic misalignment generates parasitic shear loads on the talocrural joint and discourages active patient engagement. This paper presents the design, kinematic optimization, and Assist-As-Needed (AAN) control of a wearable, three-degree-of-freedom (3-DOF) ankle rehabilitation robot built on a 3-RRR spherical parallel mechanism (SPM), whose remote center of rotation (R-CoR) is co-located with the human center of rotation (H-CoR) of the ankle. The contribution of this work is fourfold. First, we review the state of the art in robotic ankle rehabilitation, parallel-mechanism kinematics, and AAN-style training, and we use this review to motivate concrete design choices. Second, building on a prior structural design \cite{zhang2025rehab}, we perform a singularity-aware optimization of the link-length ratio and platform geometry that pushes parallel singularities toward the workspace boundary, increasing the global isotropy index of the rotational Jacobian by approximately 20\%. Third, we present analytical and numerical inverse-kinematic (IK) formulations that map a clinically meaningful (roll, pitch, yaw) command to the three actuated motor angles in real time on a Teensy~4.1 platform driving Dynamixel XM430-W350-R smart actuators over RS-485. Finally, we describe an AAN control law that estimates the patient's voluntary torque from motor-current feedback, modulates assistance accordingly, and we instrument the system with a multi-modal sensing suite (motor encoders, dual inertial measurement units, and a gait mat) to support a quantitative experimental protocol based on the Limb Symmetry Index (LSI), jerk, interaction torque, center of pressure (CoP), and the human-contribution ratio. The result is a clinically motivated and experimentally tractable platform for ankle rehabilitation research.
\end{abstract}

\begin{IEEEkeywords}
Ankle rehabilitation, spherical parallel mechanism, 3-RRR, kinematic isotropy, Assist-As-Needed control, wearable robotics, Dynamixel, gait analysis.
\end{IEEEkeywords}
...
\item a singularity-aware structural optimization of the link-length ratio and platform geometry, which moves parallel singularities toward the workspace boundary and improves the global isotropy index by approximately 20\%~(Section~\ref{sec:mechanism});
...
\item an AAN control formulation that estimates patient torque from Dynamixel current feedback and modulates assistance based on tracking error~(Section~\ref{sec:aan});
...
\item a quantitative experimental protocol, including controlled-deficit induction, transparency evaluation, and a transfer-effect study, with explicit statistical analysis plans~(Section~\ref{sec:experiments}).
\end{enumerate}

The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews related work. Section~\ref{sec:mechanism} describes the mechanism and the singularity-aware optimization. Section~\ref{sec:kinematics} develops the kinematic model. Section~\ref{sec:dynamics} summarizes the dynamic model and PID baseline. Section~\ref{sec:aan} introduces the AAN strategy. Section~\ref{sec:hardware} describes the embedded system. Section~\ref{sec:experiments} defines the experimental methodology. Section~\ref{sec:results} reports the project status. Section~\ref{sec:conclusion} concludes.
...
\end{equation}
giving a resolution of approximately $0.088^{\circ}$ per tick across the full $0$--$4095$ tick range, sufficient for the angular resolutions required in ankle rehabilitation. Figure~\ref{fig:hardware} shows representative photographs of the two principal hardware elements: the smart actuator and the prototype electronics stack, in which a Teensy~4.1 master communicates with an IMU breakout and a TTL-to-RS-485 transceiver before reaching the daisy-chain bus.

\begin{figure}[!t]
\centering
\begin{subfigure}{0.42\linewidth}
\centering
\includegraphics[width=\linewidth]{dynamixel_xm430.png}
\caption{Dynamixel XM430-W350-R}
\label{fig:hw-motor}
\end{subfigure}\hfill
\begin{subfigure}{0.55\linewidth}
\centering
\includegraphics[width=\linewidth]{electronics_breadboard.jpg}
\caption{Prototype electronics stack}
\label{fig:hw-elec}
\end{subfigure}
\caption{Embedded hardware. (a) The XM430-W350-R smart actuator, used at each of the three active joints, returns position, velocity, and current over RS-485, supporting the sensorless torque estimator of \eqref{eq:tau_ext}. (b) Prototype electronics stack on a breadboard for early integration: a Teensy~4.1 master controller (center), an IMU breakout (left) for shank/foot orientation, and a TTL-to-RS-485 transceiver (bottom) that bridges the master to the daisy-chained actuator bus.}
\label{fig:hardware}
\end{figure}
...
\section{Project Status and Implementation Plan}
\label{sec:results}

\subsection{Status of Proposed Goals}

Table~\ref{tab:goals} summarizes the status of the research goals proposed in this report.
...
\midrule
Singularity-aware structural optimization & Completed & $l_2/l_1\approx 1.15$, isotropy +20\%\\
...
AAN control formulation & Completed & Eq.~\eqref{eq:tau_ext}--\eqref{eq:tau_assist}\\
Embedded system architecture & Completed & Teensy~4.1 + RS-485 + dual IMU + gait mat\\
Hardware integration and experiments & In progress & Seven-week schedule below\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Seven-Week Implementation Plan}

The remaining work is organized as a seven-week schedule, summarized in Table~\ref{tab:milestones} and visualized in Fig.~\ref{fig:timeline} as a Gantt-style milestone strip. The schedule progresses from hardware finalization (Week~1) through low-level control integration (Week~2), AAN deployment (Week~3), the three weeks of subject experiments (Weeks~4--6), and concludes with statistical analysis and manuscript preparation (Week~7).

\begin{table}[!t]
\centering
\caption{Seven-week milestone plan toward closed-loop AAN experiments.}
\label{tab:milestones}
\begin{tabular}{cl}
\toprule
\textbf{Week} & \textbf{Milestone}\\
\midrule
1 & Finalize sensors, master controller, and AAN strategy; freeze the optimized geometry.\\
2 & Low-level Dynamixel synchronized control over RS-485; dual-IMU calibration.\\
3 & Deploy AAN controller; integrate gait-mat data parsing and CoP extraction.\\
4--6 & Run the impairment, transparency, and participation experiments; collect and analyze data.\\
7 & Consolidate results, finalize statistical analysis, and complete the manuscript.\\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Visual milestone plan for the remaining seven-week period (originally drafted as Weeks~10--16 of the AMR research course). The schedule is partitioned into hardware finalization, low-level control, algorithm deployment, two-week subject data collection, refinement based on the quantitative metrics defined in Section~\ref{sec:experiments}, and final write-up. All experiments are conducted with internal team members serving as healthy subjects under the simulated-deficit protocol of Section~\ref{sec:experiments}.}
\label{fig:timeline}
\end{figure}
...
This report extends our prior 3-RRR ankle rehabilitation robot~\cite{zhang2025rehab} from a CAD-and-prototype design into a clinically motivated, kinematically optimized, and AAN-instrumented research platform. Three contributions are reported. First, a focused review of robotic ankle rehabilitation, parallel-mechanism kinematics, AAN control, and wearable gait sensing positions the device with respect to the state of the art and motivates concrete design decisions. Second, a singularity-aware structural optimization with $l_2/l_1\approx 1.15$ pushes parallel singularities toward the workspace boundary and improves the global isotropy index by approximately 20\%. Third, an AAN control law combined with a Teensy~4.1 master controller, RS-485 daisy-chained Dynamixel actuators, dual IMUs, and an instrumented gait mat, supports a quantitative experimental protocol based on LSI, jerk, interaction torque, CoP, and the human-contribution ratio.

Future work falls into three lines. \emph{Modeling}: replace the single-center spherical assumption with a dual-axis non-concentric ankle model that better matches biomechanics, and integrate the Lagrangian dynamic model into a model-based feedforward layer. \emph{Control}: extend the AAN gain schedule with fuzzy-logic impedance and gait-phase switching, and incorporate online Jacobian-condition-number monitoring \eqref{eq:jacobian} as an adaptive velocity scaler within the safety supervisor. \emph{Experiments}: complete the seven-week protocol with healthy subjects, then plan an IRB-approved human-subject study to evaluate alignment, comfort, and therapeutic efficacy in patient populations with foot drop, post-stroke ankle weakness, or chronic ankle instability.
