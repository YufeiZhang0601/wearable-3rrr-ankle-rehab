...
\title{Design and Validation of a Wearable Ankle Rehabilitation Robot with an Anatomically Aligned 3-RRR Spherical Parallel Mechanism}
...
\textit{Code:} \url{https://github.com/YufeiZhang0601/wearable-3rrr-ankle-rehab}}
\thanks{This report was prepared for the AMR Research Course, May 2026. The full source code---URDF, parallel inverse-kinematic solver, ROS\,2 visualization and Gazebo simulation packages, parameter files, and demo scripts---is publicly released under an MIT license at the URL above.}
}
...
\item \textbf{a complete open-source design and validation pipeline}---a ROS\,2 package with a URDF / Xacro model verified in Gazebo, an interactive parallel-IK pose demo, and a dependency-free closed-loop tracking demo---all released under an MIT license and intended to be reused on the physical prototype~(Section~\ref{sec:opensource});
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.55\linewidth]{electronics_breadboard.jpg}
\caption{Prototype electronics stack on a breadboard for early integration. A Teensy~4.1 master (center) talks to an IMU breakout (left) for shank/foot orientation and a TTL-to-RS-485 transceiver (bottom) that bridges to the daisy-chained Dynamixel XM430-W350-R actuator bus. The actuator current measurement returned over the same bus feeds the sensorless torque estimator of \eqref{eq:tau_ext}.}
\label{fig:hardware}
\end{figure}
...
\section{Open-Source Design and Validation Pipeline}
\label{sec:opensource}

To support reproducibility and to lower the barrier to porting this work to other hardware, the device's full software stack is publicly released under the MIT license. The release is organized as a single ROS\,2 \emph{ament\_python} package and bundles four logically distinct layers: (i) a kinematic core that implements the numerical inverse-kinematic (IK) solver of Section~\ref{sec:kinematics}; (ii) a robot description layer (URDF and a lightweight Xacro variant) together with the visual and collision meshes; (iii) a ROS\,2 layer that exposes the kinematic core as a node and provides four launch files for visualization, scripted-trajectory playback, interactive pose commands, and Gazebo bring-up; and (iv) a dependency-free scripting layer that reproduces the closed-loop demo, mines kinematic candidates from a SolidWorks STEP export, and bootstraps a first-pass IK parameter file. The same kinematic core is shared across all entry points, so behaviour is consistent between pure-Python execution, RViz visualization, Gazebo simulation, and the eventual hardware bus.

\subsection{Parallel Inverse-Kinematics Solver}

The IK solver implements the numerical formulation of Section~\ref{sec:kinematics}, summarised in Algorithm~\ref{alg:ik}. For a requested foot-plate orientation $R_{BF}$ the solver computes each platform-side point $\mathbf{P}_i^B$, then for each leg evaluates the scalar residual $f_i(q_i)$ from \eqref{eq:leg-constraint} on a coarse motor-angle bracket. Inside any bracket where $f_i$ changes sign, a bisection root finder locates the active root, and the root closest to the previous control cycle is retained to enforce branch continuity. The solver depends only on the Python standard library, which is what allows the same code to run inside a ROS\,2 node, inside a CLI demo on a host laptop, and---once a Dynamixel SDK shim is added---inside the embedded loop on the Teensy~4.1.

\begin{algorithm}[!t]
\caption{Per-cycle parallel IK solver}
\label{alg:ik}
\begin{algorithmic}[1]
\REQUIRE foot-plate orientation $(\mathrm{roll},\mathrm{pitch},\mathrm{yaw})$, prior motor angles $\boldsymbol{q}^{-}$, geometry $\{\mathbf{B}_i^B,\mathbf{u}_i^B,\mathbf{P}_i^F,L_i\}_{i=1}^{3}$
\ENSURE motor angles $\boldsymbol{q}=(q_1,q_2,q_3)$
\STATE compute $R_{BF}=R_z(\mathrm{yaw})\,R_y(\mathrm{pitch})\,R_x(\mathrm{roll})$
\FOR{each leg $i\in\{1,2,3\}$}
  \STATE $\mathbf{P}_i^B \gets R_{BF}\,\mathbf{P}_i^F$
  \STATE evaluate residual $f_i(q)=\|\mathbf{P}_i^B-\mathbf{A}_i^B(q)\|-L_i$ on a coarse grid of $q$
  \STATE pick the bracket containing $q^{-}_i$ with $\mathrm{sign}(f_i)$ change
  \STATE $q_i \gets$ bisection on $f_i$ inside that bracket
\ENDFOR
\STATE \textbf{return} $\boldsymbol{q}$
\end{algorithmic}
\end{algorithm}

\subsection{ROS\,2 Layer and Gazebo Verification}

The ROS\,2 package wraps the kinematic core into a standard \emph{robot\_state\_publisher} pipeline. Four launch files cover the typical use cases: a static-URDF visualization in RViz; a scripted ankle-rehab trajectory replayed through a JointState player; an interactive node that converts (roll, pitch, yaw) parameters into the three motor angles via Algorithm~\ref{alg:ik} and republishes the resulting JointState at a configurable rate; and a Gazebo bring-up that loads the same URDF together with a default world. All four entries share one URDF / Xacro source and one Python kinematic core, so adding a new motion is a one-file change.

The URDF is also smoke-tested inside Gazebo. The \emph{Apply Force and Torque} dialog is used to inject pure forces and torques of varying magnitudes at the centre of mass and at off-centre Cartesian application points on each link, sweeping a representative grid. We log resulting platform poses, link inertial readings, and any mesh-collision penetrations. So far this sweep has not surfaced abnormal drifts or NaN inertias, confirming that the joint axes, inertial entries, and mesh references exported from SolidWorks are physically consistent. Figure~\ref{fig:gazebo-ft} shows the test in progress with a $\sim 100$~Nm pure torque applied to the proximal link.

\begin{figure}[!t]
\centering
\includegraphics[width=0.92\linewidth]{gazebo_force_torque.png}
\caption{Gazebo verification of the URDF / Xacro model. The \emph{Apply Force and Torque} dialog is being used to inject a 101~Nm pure torque (with components $T_x{=}11.3$, $T_y{=}{-}14.3$, $T_z{=}{-}99.3$~Nm) on the proximal link at its centre of mass. The purple arrow in the viewport is Gazebo's visualization of the applied torque, the small wireframe widget marks the application point, and the lower-left property panel reports the link pose (roll, pitch, yaw of $-1.152$, $-0.292$, $-0.124$~rad) together with the inertial entries that the URDF defines. Both are cross-checked against the SolidWorks export.}
\label{fig:gazebo-ft}
\end{figure}

\subsection{Closed-Loop Tracking Demo}

The closed-loop tracking demo, summarised in Algorithm~\ref{alg:loop}, exercises the IK solver inside a slow, low-amplitude reference trajectory on the URDF model, without requiring Gazebo, ROS\,2, or hardware. At each control cycle it generates a clinically motivated reference pose---combined sinusoidal dorsiflexion / plantarflexion at $\pm 12^{\circ}$ and inversion / eversion at $\pm 6^{\circ}$ at $0.05$~Hz, with a $3$~s soft start---runs Algorithm~\ref{alg:ik} to obtain the three motor commands, applies the safety supervisor's torque ($1.2$~Nm) and velocity ($0.8$~rad/s) caps on the per-cycle increment, and updates a virtual actuator state. Per-cycle feedback (commanded vs.\ measured joint angles, residual tracking error, and Dynamixel goal ticks via the encoding of Section~\ref{sec:hardware}) is logged to a CSV file. A screen capture of the demo running against the URDF model is included in the repository (under \emph{media/}, $\sim 25$~MB).

\begin{algorithm}[!t]
\caption{Closed-loop ankle-rehab tracking demo}
\label{alg:loop}
\begin{algorithmic}[1]
\REQUIRE URDF, amplitudes $A_p,A_r$, frequency $f$, soft-start $T_s$, torque cap $\tau_{\max}$, velocity cap $\dot{q}_{\max}$
\STATE parse URDF; identify $(j_1,j_4,j_6)$ as the three Dynamixel actuators
\STATE $\boldsymbol{q}\gets\mathbf{0}$;\ \ $t\gets 0$
\WHILE{run}
  \STATE $\sigma(t)\gets\min(t/T_s,\,1)$ \hfill\COMMENT{soft-start ramp}
  \STATE $\mathrm{pitch}\gets \sigma(t)\,A_p\sin(2\pi f t)$
  \STATE $\mathrm{roll} \gets \sigma(t)\,A_r\sin(2\pi f t)$
  \STATE $\boldsymbol{q}^{\mathrm{ref}}\gets$ \textsc{ParallelIK}(roll, pitch, $0$)
  \STATE $\Delta\boldsymbol{q}\gets$ \textsc{Clamp}($\boldsymbol{q}^{\mathrm{ref}}-\boldsymbol{q}$,\ $\dot{q}_{\max}\Delta t$)
  \STATE $\boldsymbol{\tau}\gets$ \textsc{Clamp}(controller$(\Delta\boldsymbol{q})$,\ $\tau_{\max}$)
  \STATE $\boldsymbol{q}\gets\boldsymbol{q}+\Delta\boldsymbol{q}$ \hfill\COMMENT{advance virtual actuator state}
  \STATE log $(t,\boldsymbol{q}^{\mathrm{ref}},\boldsymbol{q},\boldsymbol{\tau},\,\textsc{ToTicks}(\boldsymbol{q}))$
...
A new user reproduces the simulation results in three steps: (i) clone the repository; (ii) run the closed-loop demo as a standalone Python script for an end-to-end sanity check; and (iii) build the package in a ROS\,2 workspace (Humble or later) and launch the interactive parallel-IK pose demo for live RViz feedback. The geometry of the legs and the motor zero offsets are isolated in JSON parameter templates, so porting the same pipeline to a different physical prototype does not require touching any Python code.
